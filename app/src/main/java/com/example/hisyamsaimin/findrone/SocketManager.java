...
public class SocketManager {
    ...
    private void checkSocket(Socket tmp){
        String tmp_addr = tmp.getInetAddress().getHostAddress();
        if(tmp_addr.equals(DRONE1_ADDR)){
            drone_1 = tmp;
            try {
        ...
        else if(tmp_addr.equals(DRONE2_ADDR)){
            drone_2=tmp;
            try {
    ...
    public boolean send_to_drone1(String message) throws IOException{
        if(drone_1 == null)
            return false;

        out_drone1.println(message);
        return true;
    }

    public boolean send_to_drone2(String message) throws IOException{
        if(drone_2 == null)
            return false;

        out_drone2.println(message);
        return true;
    }
    ...
    private void message_handler(String droneID, String message){
